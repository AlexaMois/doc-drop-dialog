 ...
 */
/**
 * Простая загрузка через REST без таймаутов, ретраев и AbortController.
 * Любая дополнительная логика только маскировала зависания.
 * Браузер сам управляет временем ожидания соединения.
 */
 ...
 *
 * Защита от двойных кликов и параллельных вызовов реализована на уровне
 * вызывающего кода (isSubmitting в DocumentForm). Здесь дедупликации нет —
 * она маскировала зависания и не давала видеть, что происходит.
 */
